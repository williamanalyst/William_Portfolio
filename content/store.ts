...
import { normalizeContent, type SiteContent } from "./validation";
...
export async function readContent(): Promise<ContentState> {
  ...
  return { draft: normalizeContent(JSON.parse(row.draft)), published: normalizeContent(JSON.parse(row.published)), revision: row.revision, updatedAt: row.updated_at, publishedAt: row.published_at };
}
